...
function CarePlanCtrl($scope, $http,$log, toaster, $modal, $stateParams,addTextDomeFtory,EditCareInfoFtory, AddCareRemindFtory,AddMedicationFtory,AddSurveyFtory, AddCareCheckRemindFtory, AddLifeQualityFtory, AddCheckDocReplyFtory, AddIllnessTrackFtory) {
        ...
        $scope.planData.schedulePlans.push({
            dateSeq: function() {
                if (!$scope.planData.schedulePlans || $scope.planData.schedulePlans.length < 1)
                    return 1;
                return $scope.planData.schedulePlans[$scope.planData.schedulePlans.length - 1].dateSeq - 0 + 1;
            }()
        })
    };
    ...
    // 打开修改日程天数modal
    $scope.funEditDate = function(schedulePlan, executeTime, schedulePlansArr) {
        var modalInstance = $modal.open({
            animation: true,
            templateUrl: 'editDateModalContent.html',
            controller: 'editDateModalContentCtrl',
            size: 'md',
            resolve: {
                item: function() {
                    return {
                        ...
                        schedulePlansArr: schedulePlansArr
                    };
                }
            }
        });
        modalInstance.result.then(function(item) {
            if(item != 'cancel') {
                if(!schedulePlan.id) {
                    schedulePlan.dateSeq = item;
                } else {
                    $scope.changeSeq(schedulePlan.id, item);  
                }    
            }
        });
    };
    ...
    // 删除关怀项
    $scope.deleteCareItem = function(careItemId,sourceId) {
        if (sourceId) {
                toaster.pop('error', null, '复制项目不支持删除');
                return;
        } else{
    ...
    // 生活量表重复
    $scope.copyLifeQuality = function(careItem, sendTime,day) {
    ...
    // 病情跟踪问题重复提问
    $scope.repeatAsk = function(question, sendTime,day) {
        var repeatAskData = {
            question: question,
            sendTime: sendTime,
            thisExecuteTime:day
...
angular.module('app').controller('editDateModalContentCtrl', funEditDateModalContentCtrl);

funEditDateModalContentCtrl.$inject = ['$scope', '$http', '$modal', '$modalInstance','toaster','item'];

function funEditDateModalContentCtrl($scope, $http, $modal, $modalInstance, toaster, item) {
  
    $scope.item = {
        dateSeq: item.dateSeq,
        unit: '天'
    };
    $scope.executeTime = item.executeTime;

    // 保存
    $scope.ok = function() {
        if(!$scope.item.dateSeq || $scope.item.dateSeq == '0') {
            toaster.pop('error', null, '请填写日程日期');
            return ;
        }

        
        var dateSeq = $scope.item.dateSeq;
        if($scope.item.unit == '周') {
            dateSeq = dateSeq * 7;
        }
        if($scope.item.unit == '月') {
            dateSeq = dateSeq * 30;
        }

        if(dateSeq == item.dateSeq) {
            toaster.pop('error', null, '日程没有改变，无需修改');
            return ;
        }

        if(item.executeTime &&　dateSeq > item.executeTime) {
            toaster.pop('error', null, '日程日期必须小于或等于执行时长');
            return ;
        }

        if(item.schedulePlansArr.some(function(val) {
            return val.dateSeq == dateSeq;
        })) {
            toaster.pop('error', null, '所选日程已有日程安排！');
            return ;
        }
        $modalInstance.close(dateSeq);
    }
