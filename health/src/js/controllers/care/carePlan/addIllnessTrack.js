...
(function () {
    ...
    function CreateIllnessTrackCtrl($scope, $http, $modal, $modalInstance, toaster, illnessTrackData, editableThemes, editableOptions, Lightbox) {
        ...
        if (illnessTrackData.question) {
            ...
            $scope.editStatus = true; // 是否是编辑
            if (illnessTrackData.question.type) {
                $scope.illnessTrackData.type = illnessTrackData.question.type; // 编辑时记录单选/多选, 与新增保持一致
            }
        } else {
            $scope.illnessTrack = {
                // 'id': '', //修改时传入
        ...
        $scope.collect = function () {
            ...
            var json = {
                ...
                'type': $scope.illnessTrackData.type,
    ...
    function RepeatAskCtrl($scope, $http, $modal, $modalInstance, toaster, repeatAskData) {
        ...
        $scope.endCondition.continueDays=0;
        $scope.endCondition.level=1;
        }
        ...
        (function getReaptList() {
            $http.get(app.urlRoot + 'designer/illnessTrack/getQuestionRepeatAsk?access_token=' + app.url.access_token + '&questionId=' + repeatAskData.question.id)
                .then(function (data) {
                    data = data.data;
                    if (data && data.resultCode == 1) {
                        if (data.data!=undefined) {
                            if(data.data.endCondition){
                                $scope.endCondition = data.data.endCondition;
                            }
                            $scope.periodNum = data.data.repeatFrequency.periodNum;
                            $scope.periodUnit = data.data.repeatFrequency.periodUnit;
                            $scope.durationNum = data.data.repeatFrequency.durationNum;
                            $scope.durationUnit = data.data.repeatFrequency.durationUnit;
                            // $scope.endCondition.continueDays=data.endCondition.continueDays.toString();
                            // $scope.endCondition.level=data.endCondition.level.toString();
                            // $scope.endCondition=JSON.stringify(data.data.endCondition);
                        }
        ...
        $scope.ok = function () {
            ...
            $http.post(app.urlRoot + 'designer/illnessTrack/saveQuestionRepeatAsk', {
                access_token: app.url.access_token,
