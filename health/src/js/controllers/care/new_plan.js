...
//新建关怀计划
angular.module('app').controller('NewCarePlanCtrl', NewCarePlanCtrl);

NewCarePlanCtrl.$inject = ['$rootScope', '$scope', '$state', '$http', '$compile', 'utils', '$modal', 'toaster'];

function NewCarePlanCtrl($rootScope, $scope, $state, $http, $compile, utils, $modal, toaster) {
    var med_ids = [];
    ...
    $scope.showPop = function(tabNbr) {
        ...
        if (tabNbr !== 4) {
            ...
            $scope.medicineDialog(med_ids, function(list) {
                var dp4Ids = [];
...
//添加按钮弹窗
angular.module('app').controller('addPopCtrl', addPopCtrl);

addPopCtrl.$inject = ['$rootScope', '$scope', '$modalInstance', '$http', 'utils', 'toaster', 'data'];

function addPopCtrl($rootScope, $scope, $modalInstance, $http, utils, toaster, data) {
