...
app.controller('SigninFormController', ['$scope', '$http', '$state', '$cookieStore', 'utils', '$rootScope', 'modal',
    function($scope, $http, $state, $cookieStore, utils, $rootScope, modal) {
        ...
        function setLocalGroupList(dt) {
            ...
            var listStr = '[';
            for (var i = 0; i < len; i++) {
                listStr += '{name:"' + dt[i].name + '",id:"' + dt[i].id + '",orgType:"' + (dt[i].orgType || '') + '"';
        ...
        // 是否是集团的管理员
        function isGroupAdmin(funA, funB) {
            $http({
                method: 'post',
                url: app.url.yiliao.isAdminOfGroup,
                data: {
        ...
        $scope.login = function() {
            ...
            if ($scope.login_with_pswd) {
                ...
                //param.password = hex_md5($scope.user.password);
            } else {
            ...
            $http({
                ...
                if (response.data.resultCode === 1) {
                    ...
                    var accessGroup = function() {
                        ...
                        utils.localData('userStatus', response.data.data.user.status);
                        ...
                        if (_remember.prop("checked")) {
                            ...
                            //utils.localData('user_password', hex_md5($scope.user.password));
                        } else {
                        ...
                        // 组织判断(医院／集团)
                        var _groupList = response.data.data.user.groupList;
                        var _hospitalList = response.data.data.user.hospitalList;
                        var _hospitalList_index = _hospitalList.length;

                        if (_hospitalList_index > 0) {
                            while (_hospitalList_index--)
                                _hospitalList[_hospitalList_index].orgType = 1;
                        }

                        // 医院与集团数据并列一起
                        _groupList = _hospitalList.concat(_groupList);

                        $rootScope.groupList = _groupList || [];
                        var len = $rootScope.groupList.length;

                        //var isShowGroupLen=($rootScope.groupList.filter(function(item,index,array){
                        ...
                        if (len > 1) {
                            ...
                            if (response.data.data.user.hospitalList[0] && response.data.data.user.hospitalList[0].groupId)
                                $scope.curGroupId = response.data.data.user.hospitalList[0].groupId;
                    ...
                    if ((response.data.data.user.status === 7 && response.data.data.user.hospitalStatus != 1)) {
                        isGroupAdmin(accessGroup, function() {
                            $scope.authError = '此账号还未认证！';
                        });
                    } else if ((response.data.data.user.status === 2 && response.data.data.user.hospitalStatus != 1)) {
                        isGroupAdmin(accessGroup, function() {
                ...
                } else if (response.data.resultCode === 1040103) {
                    modal.confirm(null, '您还未设置密码！', function() {
                        $rootScope.phone = $scope.user.telephone;
                        $state.go('access.resetPassword');
                    }, { OK: '设置密码' });
                } else {
                    $scope.authError = response.data.resultMsg;
                }
