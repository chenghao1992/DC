'use strict';
(function(){
    app.controller('InvitePatientList',funcInvitePatientList);
    funcInvitePatientList.$inject=['$rootScope', '$scope', '$state', '$timeout', '$http', 'utils', 'Doctor', 'modal'];
    function funcInvitePatientList($rootScope, $scope, $state, $timeout, $http, utils, Doctor, modal) {
            ...
            setTable = function (dt) {
                ...
                dTable = doctorList.dataTable({
                    ...
                    "columns": [{
                        ...
                        "searchable": false,
                        "render": function (set, status, dt) {
                            return (set == '1' ? '正常' : set == '4' ? '暂时禁用' : set == '5' ? '永久禁用' : set == '6' ? '未激活' : set == '9' ? '已注销' : '');
                        }
                    }]
                });
            };
