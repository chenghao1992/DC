...
angular.module('app').controller('ReportsOfFinance', ReportsOfFinance);

ReportsOfFinance.$inject = ['$rootScope', '$scope', '$state', '$timeout', '$http', 'utils', '$stateParams', 'JQ_CONFIG', 'uiLoad', 'modal',  'AppFactory', 'Group'];
function ReportsOfFinance($rootScope, $scope, $state, $timeout, $http, utils, $stateParams, JQ_CONFIG, uiLoad, modal, AppFactory, Group) {
        ...
        //获取选择的月份的第一天和最后一天
        var selectStartTime = new Date(Date.parse(flg.replace('年', '-').replace('月', '-').replace('日', '')));
        var selectEndTime = new Date(selectStartTime);

        selectEndTime.setMonth(selectStartTime.getMonth() + 1);

        selectEndTime.setDate(0);
        //var selectEndTime=new Date(selectStartTime.getTime()-1000*60*60*24);
        ...
        // 初始化表格
        var table, dTable, keyWord, index, length, total;
            ...
            var setTable = function() {
                ...
                dTable = table.dataTable({
                    ...
                    "fnServerData": function(sSource, aoData, fnCallback) {
                        if (param.startTime && typeof param.startTime !== "number") {
                            param.startTime = Date.parse(param.startTime);
                            /*if(!param.endTime){
                                param.endTime = new Date().getTime();
                            }*/
                        }
                        if (param.endTime && typeof param.endTime !== "number") {
                            param.endTime = Date.parse(param.endTime) + 24 * 60 * 60 * 1000 - 1000;
                        }
                ...
                function setSearchBar() {
                    if ($scope.isGroup && flg && flg != '0') {
                        ...
                        uiLoad.load(JQ_CONFIG.dateTimePicker).then(function() {
                            var now = new Date();
                            var startDate = flg.replace(/\D/g, '/') + '01';
                            var start_time = $("#start_time").datetimepicker({
                                ...
                                start_time.datetimepicker('setStartDate', selectStartTime);
                                start_time.datetimepicker('setEndDate', selectEndTime);
                                //start_time.datetimepicker('setEndDate', now);

                                //  start_time.datetimepicker('setStartDate', startDate);
                            ...
                            var end_time = $("#end_time").datetimepicker({
                                ...
                                end_time.datetimepicker('setEndDate', selectEndTime);
                            }).on('changeDate', function() {
