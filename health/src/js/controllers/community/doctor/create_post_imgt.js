...
(function() {
    ...
    function creatImgtCtrl($scope, $rootScope, $timeout, utils, $http, $modal, Upload, toaster, $state, $stateParams, Group, AppFactory, $sce, MyFileSelectBoxFactory, ShowData) {
        ...
        $scope.isSaveDoc = false;
        $scope.$watch('formData.content', function(newValue, oldValue) {
            if (newValue != oldValue) {
                //得到富文本编辑器纯文本
                var txt = $scope.umeditor.getContentTxt().trim();
                var umeditorLength = txt.length;
                if (umeditorLength > 0) {
                    $scope.isSaveDoc = true;
                } else {
                    $scope.isSaveDoc = false;
                }
        ...
        $scope.saveDoc = function(saveType) {
            ...
            if ($scope.uploadPercent&&!($scope.uploadPercent == 0 || $scope.uploadPercent == 100)) {
                toaster.pop('warn', '', '正在上传图片中，请稍后');
                return;
            }
            $scope.isSaved = true;
            var digest = '';
            var contentTxt = $scope.umeditor.getContentTxt().trim();
            if (contentTxt) {
