(function() {
    ...
    ChangePwdModalInstanceCtrl.$inject = ['$scope', '$uibModalInstance', 'toaster', '$http','constants'];

    function ChangePwdModalInstanceCtrl($scope, $uibModalInstance, toaster, $http,constants) {
        $scope.ok = function () {
            $scope.errorInfo='';
            ...
            $http.post(constants.api.shared.updatePassword, {
                access_token: localStorage['enterprise_access_token'],
                newPassword:$scope.newPassword,
                oldPassword:$scope.curPassword,
                userId:JSON.parse(localStorage.getItem('enterprise_user')).id
            }).
