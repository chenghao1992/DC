(function() {
    ...
    DoctorInfoDailogCtrl.$inject = ['$scope', '$uibModal','constants'];

    function DoctorInfoDailogCtrl($scope, $uibModal,constants) {
    ...
    DoctorInfoDailogModalInstanceCtrl.$inject = ['$scope', '$uibModalInstance', 'DoctorInfoDailogFtory', 'moment', 'toaster', 'doctorId', 'goRunning', 'data','$http','constants'];

    function DoctorInfoDailogModalInstanceCtrl($scope, $uibModalInstance, DoctorInfoDailogFtory, moment, toaster, doctorId, goRunning, data,$http,constants) {
        ...
        function doctorInfo(doctorId) {
            ...
            function thenFc(response) {
                ...
                if(response.packList&&response.packList.length>0){
                    $scope.messageCon=response.packList.find(function(item){
                        return item.packType==1;
                    });
                    $scope.telCon=response.packList.find(function(item){
                        return item.packType==2;
                    });
                    var careConList=[];
                    $scope.careCon=null;
                    careConList=response.packList.filter(function(item){
                        return item.packType==3;
                    });
                    if(careConList.length>0){
                        careConList.sort(function(a,b){
                            return a.price-b.price;
                        });
                        $scope.careCon=careConList[0];
                    }

                    console.log($scope.messageCon,$scope.telCon,$scope.careCon);
                }
        ...
        $scope.sendAbort=function(type){
            $scope.disableSend=true;
            $scope.appointTimeLoading=true;
            $http.post(constants.api.order.sendCardEvent, {
                access_token:localStorage['guider_access_token'],
                type: type,
