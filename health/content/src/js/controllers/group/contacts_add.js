'use strict';
(function(){
    app.controller('ContactsAdd',funcCaCtrl);
    funcCaCtrl.$inject=['$rootScope', '$scope', '$state', '$timeout', '$http', 'utils', 'modal'];
    function funcCaCtrl($rootScope, $scope, $state, $timeout, $http, utils, modal) {
        ...
        function check(info) {
            ...
            if(info.pid != '0' && info.pid && info.pid != 'x'){
                modal.toast.warn('自定义组织不能超过两级!');
                i.trigger('click');
                return;
            }
        }
