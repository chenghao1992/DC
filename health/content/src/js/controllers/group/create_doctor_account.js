'use strict';
(function(){
    app.controller('CreateDoctorAccount',funcCdaCtrl);
    funcCdaCtrl.$inject=['$rootScope', '$scope', '$state', '$timeout', '$http', 'utils', 'modal', '$modal', '$compile'];
    function funcCdaCtrl($rootScope, $scope, $state, $timeout, $http, utils, modal, $modal, $compile) {
        $scope.groupId = utils.localData('curGroupId') || '666666666666666666666666';
        $scope.hospital = {};
        ...
        function initChosen(dt) {
            ...
            if (!isExist && $scope.title) {
                opt = $('<option selected>' + $scope.title + '</option>');
                tmp.prepend(opt);
            }else if (!isExist){
                opt = $('<option selected value="0">请选择职称</option>');
                tmp.prepend(opt);
            }
        ...
        $scope.submitInfo = function () {
            $http({
                url: app.url.hospital.registerByAdmin,
                method: 'post',
                data: {
                    access_token: app.url.access_token,
                    name: $scope.userName,
                ...
                if (resp.data.resultCode === 1) {
                    if (resp.data.data.status === 0) {
                        ...
                        var div = $('<div></div>');
                        var str = '<p class="text-success">新建成功！</p><br/><p class="text-normal">用户可以使用手机号登陆app，<br/>密码默认为空，通过“找回密码”重置登陆密码。<br/><br/><label class="checkbox i-checks">' +
                        ...
                        modal.prompt(null, div, function () {
                            ...
                            if($scope.willSendMsg) {
                                $http({
                                    url: app.url.hospital.registerByGroupNotify,
                                    method: 'post',
                                    data: {
                                        access_token: app.url.access_token,
                        ...
                        modal.confirm(null, '该手机号已注册，是否发送短信邀请医生加入集团？', function () {
                            // 发送邀请短信
                            $http({
                                url: app.url.hospital.saveBatchInvite,
                                method: 'post',
                                data: {
                                    access_token: app.url.access_token,
                                    groupId: $scope.groupId,
                    ...
                    modal.toast.error(resp.data.resultMsg||'添加失败');
                }
            }, function (resp) {
                //$scope.authError = resp.data.resultMsg;
        ...
        $scope.uploaderSuccess = function (up, file, info) {
            if (file.url) {
                if(!/.(gif|jpg|jpeg|png|gif|jpg|png)$/.test(file.type)){
                    modal.toast.error('图片格式不正确！');
                    $scope.usrPicProgress=0;
                    $scope.$apply();
                    return;
                }
...
(function(){
    app.controller('AddHospitalCtrl',funcAhCtrl);
    funcAhCtrl.$inject=['$http', '$modalInstance', '$scope', 'toaster'];
    function funcAhCtrl($http, $modalInstance, $scope, toaster) {
        // 获取医院级别
