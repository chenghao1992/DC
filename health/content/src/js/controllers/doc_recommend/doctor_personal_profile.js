...
angular.module('app').controller('DoctorPersonalProfileCtrl', DoctorPersonalProfileCtrl);

DoctorPersonalProfileCtrl.$inject = ['$scope', 'utils', '$http', '$modal', 'toaster', '$location', '$state', '$rootScope', '$stateParams'];

function DoctorPersonalProfileCtrl($scope, utils, $http, $modal, toaster, $location, $state, $rootScope, $stateParams) {
    $scope.isSaved = false;
    $scope.isShow=false;
    $scope.name=$stateParams.name;
    //编辑H5
    function getData() {
        $http.post(app.url.doctor.getRecommentDoc, {
    ...
    $scope.save = function() {
        ...
        $http.post(app.url.doctor.updateRecDocument, {
            access_token: app.url.access_token,
            recommendDoctId:$stateParams.id,
            doctorName:$stateParams.name,
        ...
        success(function(data, status, headers, config) {
            if (data.resultCode == 1) {
                ...
                setTimeout(function() {
                    $state.go('app.docRecommend.list', {}, {
                        reload: true
                    });
                }, 3000)
