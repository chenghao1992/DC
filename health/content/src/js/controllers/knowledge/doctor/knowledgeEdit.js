...
'use strict';
(function() {
    angular.module('app').controller('doctorKnowledgeEditCtrl', funGroupKnowledgeEditCtrl);

    funGroupKnowledgeEditCtrl.$inject = ['$scope', '$http', '$state', 'toaster', '$stateParams', 'Group', 'AppFactory', '$modal'];

    function funGroupKnowledgeEditCtrl($scope, $http, $state, toaster, $stateParams, Group, AppFactory, $modal) {
        ...
        (function() {
            ...
            success(function(data) {
                if (data.resultCode == 1) {
                    $scope.formData.selectedType = data.data[data.data.length - 1];
        ...
        $scope.updateKnowledge = function() {
            ...
            success(function(data) {
                if (data.resultCode == 1) {
                    if (!data.data.status) {
                        toaster.pop('success', null, data.data.msg);
                        return;
                    }
                    ...
                    toaster.pop('error', null, data.msg);
                }
            }).
            error(function(data) {
                //toaster.pop('error', null, data.resultMsg);
        ...
        $scope.deleteKnowledge = function() {
            ...
            success(function(data) {
                if (data.resultCode == 1) {
                    ...
                    $state.go('app.doctorKnowledge.list', {}, { reload: true });
