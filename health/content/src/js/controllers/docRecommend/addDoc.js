...
'use strict';
(function() {
    angular.module('app').controller('addDocRecommendCtrl', funAddDocRecommendCtrl);

    funAddDocRecommendCtrl.$inject = ['$scope', '$http', '$state', 'toaster'];

    function funAddDocRecommendCtrl($scope, $http, $state, toaster) {
            $scope.isSaved = false;
    $scope.isShow=false;
    ...
    $scope.save = function() {
        ...
        success(function(data, status, headers, config) {
            if (data.resultCode == 1) {
                ...
                setTimeout(function() {
                    $state.go('app.doc_recommend.list', {}, {
                        reload: true
                    });
                }, 3000);
