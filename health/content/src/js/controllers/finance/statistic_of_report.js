...
angular.module('app').controller('StatisticOfReport', StatisticOfReport);

    StatisticOfReport.$inject = ['$rootScope', '$scope', '$state', '$timeout', '$http', 'utils'];

    function StatisticOfReport($rootScope, $scope, $state, $timeout, $http, utils) {
    ...
    $scope.seeDetails = function (date) {
        if (date) {
            $state.go('app.finance.reports', {name: 'group', date: date}, {reload: false})
        }
    };
