            ...
            this.getAsyncData = function(fun){
                ...
                $http({
                    ...
                    if(resp.data.data){
                        ...
                        info.inviter = (data.inviteRelation && data.inviteRelation.name) ? data.inviteRelation.name : '';
                        ...
                        info.appointmentGroupProfit = exist ? dt.appointmentGroupProfit : '';
                        info.appointmentParentProfit = exist ? dt.appointmentParentProfit : '';
                        info.chargeItemGroupProfit = exist ? dt.chargeItemGroupProfit : '';
                        info.chargeItemParentProfit = exist ? dt.chargeItemParentProfit : '';
                        ...
                        if(data.user && data.user.doctor){
                            info.area = {
                                province: data.user.doctor.province || '',
                                city: data.user.doctor.city || '',
                                country: data.user.doctor.country || ''
                            };
                        }

                        info.departments = (data.user && data.user.doctor && data.user.doctor.departments) ? data.user.doctor.departments : '';
                        info.service = data.packName ? data.packName : [];
