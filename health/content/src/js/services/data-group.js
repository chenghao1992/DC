        ...
        function($rootScope, $http, utils, modal) {
            ...
            this.init = function(dt) {
                ...
                if (dt) {
                    ...
                    info.standard = dt.standard || '';
                    info.creatorName = dt.creatorName || '';

                    info.user.id = (dt.groupUser && dt.groupUser.doctorId) || (dt.groupUser && dt.groupUser.id) || '';
                    ...
                    info.isBlocked = dt.skip || info.skip;
            ...
            this.update = function(id,fun) {
                getAsyncData(id, fun);
            };


            // 获取当前的组织信息
            this.getCurrentOrgInfo = function() {

                var _groupList = window.eval(localStorage.getItem('groupList'));
                var _groupList_index = _groupList.length || [];

                while (_groupList_index--) {

                    if (localStorage.getItem('curGroupId') == _groupList[_groupList_index].id)
                        return _groupList[_groupList_index];
