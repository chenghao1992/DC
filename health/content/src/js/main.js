...
(function(){
    ...
    AppCtrl.$inject=['$rootScope', '$scope', '$translate', '$localStorage', '$window', '$http', '$state', '$cookieStore', 'utils', 'Group', 'modal'];
    function AppCtrl($rootScope, $scope, $translate, $localStorage, $window, $http, $state, $cookieStore, utils, Group, modal) {
            ...
            $http.post(app.url.yiliao.verifyByGuser, {
                ...
                if (resp.resultCode != 1) {
                    ...
                    if (!resp.data.group) {
                        ...
                        $rootScope.isSysAdmin = false; // 是否为管理员
                        Group.init(null); // 设置集团相关的数据集合
                    ...
                    // 审核未通过时，只能登录身份为管理员的集团
                    if (resp.data.userStatus == '4') {
                        modal.toast.warn('您的账号<span class="text-danger-dker">未通过审核</span>，只能登录身份为<span class="text-success-dker">管理员</span>的集团', 7000);
                        return;
                    }
                    ...
                    var dt = {
                        ...
                        creatorName: resp.data.group.creatorName,
                    ...
                    Group.handle(dt.id, function (group) {
                        ...
                        switch (group.user.status) {
                            case '1':
                                ...
                                break;
                            case '2':
                                if (group.status.active || group.status.active === 'active') {
                                    $state.go('app.invite_list', null, {
                                        reload: true
                                    });
                                } else {
                            ...
                            case '3':
                                if (!group.status.active || group.status.active === 'active') {
                                    if (group.isBlocked === 'S') {
                                        $state.go('app.groupSettings', null, {
                                            reload: true
                                        });
                                    } else {
    ...
    angular.module('app').controller('GuoupSearch',GuoupSearch);
    GuoupSearch.$inject=['$rootScope', '$scope', '$http', '$state', 'utils'];
    function GuoupSearch($rootScope, $scope, $http, $state, utils){
    ...
    angular.module('app').controller('GroupSwitch',GroupSwitch);
    GroupSwitch.$inject= ['$rootScope', '$scope', '$http', '$state', 'utils'];
    function GroupSwitch($rootScope, $scope, $http, $state, utils){
                ...
                if (groupId != curGroupId) {
                    if ((groupId && userId) || (groupId && userId == '0') || (groupId == '0' && userId)) {
                        ...
                        for (var i = 0; i < groupList.length; i++) {
                            listStr += '{name:"' + groupList[i].name + '",id:"' + groupList[i].id + '",orgType:"' + (groupList[i].orgType || '') + '"';
...
// angular.module('app').controller('AppCtrl', ['$rootScope', '$scope', '$translate', '$localStorage', '$window', '$http', '$state', '$cookieStore', 'utils', 'Group', 'modal',
//     function($rootScope, $scope, $translate, $localStorage, $window, $http, $state, $cookieStore, utils, Group, modal) {
//
//         //var dt = Group.getData();
//         // add 'ie' classes to html
//         var isIE = !!navigator.userAgent.match(/MSIE/i);
