(function () {
    app.directive('doctorDetails', ['uiLoad', 'JQ_CONFIG', '$document', '$http', 'Doctor', '$state', '$stateParams', 'modal', 'utils', 'Group', 'AppFactory',
        function(uiLoad, JQ_CONFIG, $document, $http, Doctor, $state, $stateParams, modal, utils, Group, AppFactory) {
            return {
                restrict: 'E',
                ...
                link: function(scope, el, attr) {
                    ...
                    scope.tabs.show = function() {
                        if (firstVisit) {
                            Doctor.getAsyncData(function(dt) {
                                ...
                                scope.viewData.inviter = data.inviter;
                                scope.viewData.userStatus = data.state == '1' ? '审核通过' : data.state == '2' ? '待审核' : data.state == '3' ? '审核未通过' : '未认证';
                                ...
                                scope.viewData.area = data.area.province + ' - ' + data.area.city + ' - ' + data.area.country;
                                scope.viewData.departments = data.departments;
                                scope.viewData.services = data.service.join(', ');
                    ...
                    scope.tabs.setProfit = function() {
                        ...
                        Group.handle(data.groupId, function(group) {
                            // 是否博徳嘉联
                            if (group && AppFactory) {
                                AppFactory.group.isServiceGroup(group.id).then(function (_data) {
                                    if (_data) {
                        ...
                        if(!scope.isServiceGroup){
                            ...
                            profits = [
                                'formData.appointmentParentProfit',
                                'formData.appointmentGroupProfit',
                                'formData.chargeItemParentProfit',
                                'formData.chargeItemGroupProfit',
                                'formData.textParentProfit',
                                'formData.textGroupProfit',
                                'formData.phoneParentProfit',
                                'formData.phoneGroupProfit',
                        ...
                        scope.formData.appointmentGroupProfit = data.appointmentGroupProfit;
                        scope.formData.appointmentParentProfit = data.appointmentParentProfit;
                        scope.formData.chargeItemGroupProfit = data.chargeItemGroupProfit;
                        scope.formData.chargeItemParentProfit = data.chargeItemParentProfit;
                        ...
                        // 保存标签页中的数据，用于检测是否修改
                        dataProfit.appointmentGroupProfit = scope.formData.appointmentGroupProfit;
                        dataProfit.appointmentParentProfit = scope.formData.appointmentParentProfit;
                        dataProfit.chargeItemGroupProfit = scope.formData.chargeItemGroupProfit;
                        dataProfit.chargeItemParentProfit = scope.formData.chargeItemParentProfit;
                        ...
                        scope.saveProfit = function() {
                            if (scope.formData.appointmentGroupProfit > 100 || scope.formData.appointmentParentProfit > 100 ||
                                scope.formData.chargeItemGroupProfit > 100 || scope.formData.chargeItemParentProfit > 100 ||
                                scope.formData.textGroupProfit > 100 || scope.formData.textParentProfit > 100 ||
                            ...
                            // 设置抽成关系
                            // if (parentId && dataProfit.parentId != parentId) {
                            var param = {
                                access_token: app.url.access_token,
                                groupId: data.groupId,
                                parentId: parentId,
                            ...
                            if (dataProfit.appointmentGroupProfit != scope.formData.appointmentGroupProfit || dataProfit.appointmentParentProfit != scope.formData.appointmentParentProfit ||
                                dataProfit.chargeItemGroupProfit != scope.formData.chargeItemGroupProfit || dataProfit.chargeItemParentProfit != scope.formData.chargeItemParentProfit ||
                                dataProfit.textGroupProfit != scope.formData.textGroupProfit || dataProfit.textParentProfit != scope.formData.textParentProfit ||
                                ...
                                param = {
                                    ...
                                    groupId: data.groupId,
                                    id: scope.targetDoctorId,
                                    appointmentGroupProfit: scope.formData.appointmentGroupProfit,
                                    appointmentParentProfit: scope.formData.appointmentParentProfit,
                                    chargeItemGroupProfit: scope.formData.chargeItemGroupProfit,
                                    chargeItemParentProfit: scope.formData.chargeItemParentProfit,
                    ...
                    scope.tabs.otherInfo = function() {
                        scope.viewData.inviteCode = window.location.protocol + '//' + window.location.host + '/health/web/appInvite/joinToGroup.html?groupId=' + data.groupId + '&doctorId=' + data.doctorId;
                        scope.isLoading = false;
                        if (scope.info) {
                            scope.isLoading = false;
                            return;
                    ...
                    scope.copyText = function(e){
                        /*var text_code = $('#text_code');
                         text_code.off().on('paste', function(e) {
                         clipboardData = (event.clipboardData || window.clipboardData);      // 获取剪贴板对象
                         if(clipboardData.setData("text", scope.viewData.inviteCode)){       // 设置剪贴板文本
                         modal.toast.success('复制成功！');
                         }
                         });
                         text_code.trigger('focus');
                         text_code.trigger('paste');*/
                        utils.copyToClipboard(scope.viewData.inviteCode);
                    };
