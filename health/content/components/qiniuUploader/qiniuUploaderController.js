(function() {
    angular.module('app')
        .controller('QiniuUploaderCtrl', QiniuUploaderCtrl);

    QiniuUploaderCtrl.$inject = ['$scope', '$http'];

    function QiniuUploaderCtrl($scope, $http) {

        // 接口（POST）：
        //      http://<im服务器:8090>/im/file/getUpToken.action
        //      header：
        //         access_token：登录成功拿到的许可
        //         content-type：application/json
        //     参数：
        //          bucket:文件所在空间，默认message
        // /qiniu/im/file/getUpToken.action

        if (!$scope.token || !$scope.bucket) {

            console.error('缺少：' + $scope.token, '或者：' + $scope.bucket);
            ...
            $http({
                ...
                if (rep && rep.resultCode == 1 && rep.data && rep.data.token && rep.data.domain) {
                    qniuSetting(rep.data.token, rep.data.domain);
                } else if (rep.resultCode == 0 && rep.resultMsg) {
                    alert('获取上传token失败：' + rep.resultMsg);
                } else {
                    console.error(rep);
                }
            });
        ...
        //每次调用创建一个随机名字的上传按钮
        var uploadBtnName = 'button_' + Math.random().toFixed(3) * 1000;

        // 上传文件
        $scope.upload = function() {
            document.getElementById(uploadBtnName).click();
        };

        // 七牛上传配置
        function qniuSetting(uptoken, domain) {
            var uploadBtnCon = document.createElement('div');
            uploadBtnCon.innerHTML = '<a class="btn btn-default btn-lg " id="' + uploadBtnName + '">\
                <i class="glyphicon glyphicon-plus"></i>\
                <span>选择文件</span>\
                </a>';

            document.getElementById('container').appendChild(uploadBtnCon.firstChild);

            if (!uptoken)
                return console.log('uptoken不能为空');

            var thisQiniu = new QiniuJsSDK();

            $scope.uploader = thisQiniu.uploader({

                uptoken: uptoken,
                // uptoken_url: 'qiniu/im/file/getUpToken.action',
                domain: 'http://' + domain,

                runtimes: 'html5,flash,html4',
                browse_button: uploadBtnName,
                container: 'container',
                drop_element: 'container',
                max_file_size: $scope.maxFileSize || false,
                flash_swf_url: 'js/plupload/Moxie.swf',
                dragdrop: true,
                chunk_size: $scope.chunkSize || false,
                get_new_uptoken: false,
                auto_start: $scope.autoStart == 'false' ? false : true, //自动上传
                unique_names: false,
                save_key: false,
                multi_selection: $scope.multiSelection || false,
                filters: $scope.filters || false,

                init: {
                    'FilesAdded': function(up, files) {

                        // console.log($scope.maxSelect);
                        if ($scope.maxSelect > -1 && $scope.maxSelect < files.length) {
                            for (var i = 0; i < files.length; i++) {
                                $scope.uploader.removeFile(files[i]);
                                // 失败后执行回调
                                if ($scope.errorCallBack) {
                                    $scope.errorCallBack(up, null, '选择的文件数量大于' + $scope.maxSelect);
                                }
                                return false;
                            };
                        };
                        ...
                        if ($scope.$root.$$phase != '$apply' && $scope.$root.$$phase != '$digest') {
                            $scope.$apply('fileList');
                        };
                        // console.log($scope.fileList);
                        if ($scope.addedCallBack) {
                            $scope.addedCallBack(up, files);
                        };
                    ...
                    'BeforeUpload': function(up, file) {
                        ...
                        for (var i = 0; i < $scope.fileList.length; i++) {
                            if ($scope.fileList[i].id == file.id) {
                                $scope.fileList[i].isUploading = true;
                            }
                        };

                        if ($scope.beforeCallBack) {
                            $scope.beforeCallBack(up, file);
                        };
                    },
                    'UploadProgress': function(up, file) {
                        ...
                        // console.log(file);
                        // 已上传的大小
                        var loadedSize = plupload.formatSize(file.loaded).toUpperCase();
                        file.loadedSize = loadedSize;
                        // 上传速度
                        var formatSpeed = plupload.formatSize(file.speed).toUpperCase();
                        file.formatSpeed = formatSpeed;

                        for (var i = 0; i < $scope.fileList.length; i++) {
                            if ($scope.fileList[i].id == file.id) {
                                $scope.fileList[i].formatSpeed = formatSpeed;
                                $scope.fileList[i].loadedSize = loadedSize;
                                $scope.fileList[i].status = file.status;
                                $scope.fileList[i].error = null;
                                $scope.fileList[i].isUploading = false;
                        ...
                        if ($scope.progressCallBack) {
                            $scope.progressCallBack(up, file);
                        };
                    ...
                    'FileUploaded': function(up, file, info) {
                        ...
                        for (var i = 0; i < $scope.fileList.length; i++) {
                            if ($scope.fileList[i].id == file.id) {
                                $scope.fileList[i].status = file.status;
                                $scope.fileList[i].url = sourceLink || '';
                                break;
                            }
                        };
                        ...
                        if ($scope.successCallBack) {
                            $scope.successCallBack(up, file, info);
                        };
                    ...
                    'Error': function(up, err, errTip) {
                        ...
                        for (var i = 0; i < $scope.fileList.length; i++) {
                            if ($scope.fileList[i].id == up.id) {
                                $scope.fileList[i].status = up.status;
                                $scope.fileList[i].error = errTip;
                                break;
                            }
                        }
                        ...
                        // 失败后执行回调
                        if ($scope.errorCallBack) {
                            $scope.errorCallBack(up, err, errTip);
                        }
            ...
            $scope.cancel = function(file, index) {
                ...
                if ($scope.fileList[index].status == 5 || $scope.fileList[index].status == 4)
                    return;

                $scope.fileList[index].status = -1;
                $scope.uploader.removeFile(file);
            }
            $('#up_load_files_start').on('click', function() {
