...
(function () {
	...
	angular.module('app').service('$qupload', ['$http', '$q',
		...
		function ($http, $q) {
			...
			this.upload = function (config) {
				...
				var blockRet = localStorage.getItem(fileHashKey);
				if (!blockRet) {
					blockRet = [];
				}
				...
				var mkfile = function (file, blockRet) {
					...
					$http({
						...
						deferred.resolve(e);
						localStorage.removeItem(fileHashKey);
					}).error(function (e) {
						deferred.reject(e);
					});
				};
					...
					xhr.onreadystatechange = function (response) {
						if (response && xhr.readyState === 4 && xhr.status === 200) {
							if (xhr.status === 200) {
								...
								localStorage.setItem(fileHashKey, blockRet);
...
angular.module('app').directive('ngFileSelect', ['$parse', '$timeout',
