...
//                }
        ...
        callback: function (editor, $w, url, resultCode) {
            if (resultCode == '1') {
                //显示图片计数+1
                Upload.showCount++;
    ...
    var Upload = {
        showCount: 0,
        uploadTpl: '<div class="edui-image-upload%%" id="pickfiles">' +
            '<span class="edui-image-icon" id="html5_1apfmm8ng825vqe14ec1dirn5em"></span>' +
            //'<form class="edui-image-form" method="post" enctype="multipart/form-data" target="up">' +
            //'<input style=\"filter: alpha(opacity=0);\" style=\"display: none;\" class="edui-image-file" type="file" hidefocus name="upfile" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp"/>' +
            //'<input type="text" name="path" id="inputPath">'+
            // '</form>' +
        ...
        config: function (sel) {
            var me = this,
                ...
                path=me.editor.options.uploadPath;
                qiniuToken=me.editor.options.qiniuToken;
                qiniuDomain=me.editor.options.qiniuDomain;

            //url=url + (url.indexOf("?") == -1 ? "?" : "&") + "editorid="+me.editor.id;//初始form提交地址;

            $("form", $(sel, me.dialog)).attr("action", url);
            $("#inputPath", $(sel, me.dialog)).attr("value", path);
            return me;
        },
        uploadComplete: function(r){
            var me = this;

            Base.callback(me.editor,me.dialog,r,1);
            //var me = this;
            //try{
            //    var json = eval('('+r+')');
        ...
        submit: function (callback) {
            console.log(1212);
            var me = this,
            uploader = (new QiniuJsSDK).uploader({
                uptoken: me.editor.options.qiniuToken,
                domain: 'http://' + me.editor.options.qiniuDomain,
                runtimes: 'html5,flash,html4',
                browse_button: 'pickfiles',
                container: 'edui-image-content',
                drop_element: 'edui-image-content',
                max_file_size: '5mb',
    ...
    UM.registerWidget('image', {
        tpl: "<link rel=\"stylesheet\" type=\"text/css\" href=\"<%=image_url%>image.css\">" +
            ...
            "</ul>" +
            "<div class=\"edui-tab-content\" id=\"edui-tab-content\">" +
            "<div class=\"edui-image-local edui-tab-pane edui-active\">" +
            "<div class=\"edui-image-content\" style=\"overflow: hidden;\" id=\"edui-image-content\"></div>" +
