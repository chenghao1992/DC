'use strict';
(function () {
    app.controller('DrugADCtrl', ['$scope', '$timeout','utils','$http','modal','toaster','$location','$state','$rootScope',function($scope, $timeout,utils,$http,modal,toaster,$location,$state,$rootScope) {
        //所有文档表格
        var articleTable;
        ...
        //所有文档获取数据的url和param
        var docUrl=app.url.drug.getAllAdvertisements;
        var docParam={
        ...
        $scope.clearMainKW=function(){
            $scope.mainKeyword=null;
            docUrl=app.url.drug.getAllAdvertisements;
            docParam={
                access_token:app.url.access_token,
                documentType:1,
                pageIndex:0,
        ...
        var editArticle=function(aData){
            $state.go('app.document.edit_drug_ad',{id:aData.id},{'reload':true});
        };

        //将显示的文章上移
        var upperDoc=function(id){
            $http.post(app.url.drug.upAdvertisement, {
                access_token:app.url.access_token,
                id:id,
            }).
            success(function(data, status, headers, config) {
            ...
            var setAllTable = function () {
                articleTable = $('#article_list').DataTable({
                    ...
                    "columns": [{
                        ...
                        "render": function (set, status, dt) {
                            return utils.dateFormat(dt.createTime,'yyyy-MM-dd');
                        }
                    },
                        {
                            "render": function (set, status, dt) {
                    ...
                    "createdRow": function (nRow, aData, iDataIndex) {
                        ...
                        $(nRow).on('click', function (event) {
                            var url = $state.href('drug_ad_article', {id: aData.id});
