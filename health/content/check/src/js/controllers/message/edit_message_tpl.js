...
angular.module('app').controller('MsgTplEditCtrl', MsgTplEditCtrl);
MsgTplEditCtrl.$inject = ['$scope', '$timeout','utils','$http','$modal','toaster','$state','$stateParams','$sce'];
function MsgTplEditCtrl($scope, $timeout,utils,$http,$modal,toaster,$state,$stateParams,$sce) {
    $scope.titleLength=0;
            ...
            $http.post(app.url.msg.saveMsgTpl, {
                ...
                success(function(data, status, headers, config) {
                    if(data.resultCode==1){
                        ...
                        setTimeout(function(){
                            $state.go('app.message.tpl',{},{reload:true});
                        },3000);
                    }
                    else{
                        $scope.isSaved=false;
