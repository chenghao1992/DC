(function() {
    ...
    QuickReplyController.$inject = ['$scope', '$uibModal', 'QuickReplyFactory','utils'];

    function QuickReplyController($scope, $uibModal, QuickReplyFactory,utils) {
        ...
        function getQuickReplyList() {
            ...
            var param = {
                access_token: utils.localData('check_access_token')
            }
        ...
        $scope.addQuickReplyItem = function() {
            $scope.quickReplyList.push({
                replyContent: '',
                replyId: '',
                userId: JSON.parse(localStorage['check_user']).id,
                is_system:1
            });
