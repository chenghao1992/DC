(function() {
    angular.module('app').directive('uiMedicine', function($timeout, $rootScope, $parse, $http,constants) {
        var templateStr = '<div class="commsearch"><div class="mask"></div>' +
            ...
            '<input type="text" class="form-control" placeholder="药品名称/助记码/生产厂家/药品商品名/医药公司" ng-model="keyword" ng-keypress="pressEnter($event)">' +
            ...
            '<img class="pull-left b-3x m-r" ng-src="{{l.image_url}}" />' +
            '<div>' +
            '<div class="h3 m-t-xs m-b-xs ng-binding" ng-bind="l.general_name"></div>' +
            '<small class="text-muted ng-binding" ng-bind="l.pack_specification"></small>' +
            ...
            '.store-result-list label img {margin-top: 8px;display: inline-block;width: 60px;height: 60px;border-radius:30px;-webkit-border-radius:30px;-moz-border-radius:30px;}' +
        ...
        return {
            ...
            link: function($scope, element, attr) {
                ...
                $scope.findDrugByKeyWord = function() {
                    ...
                    var query_for_drug_company_url = "/web/api/invoke/2/" + localStorage.getItem('groupHelper_access_token') + "/c_Goods.select";
                    var set_good_state_url = "/web/api/invoke/" + localStorage.getItem('groupHelper_access_token') + "/c_GoodsOnStack.set_good_state";
                    var url = '';
                    if (typeof key == 'undefined') {
                        url = query_for_drug_company_url;
                    } else {
                        url = query_for_drug_company_url + "?__KEYWORD__=" + key
                    }
