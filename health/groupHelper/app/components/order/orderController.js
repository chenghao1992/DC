...
(function() {
    ...
    // 手动注入依赖
    orderCtrl.$inject = ['$scope', '$interval', '$timeout', '$filter', '$sce', '$anchorScroll', '$location', '$state', 'Lightbox', '$http', 'ngAudio', 'OrderFactory', 'toaster', 'ChatImgSelModalFactory', 'constants','searchOrderModalFactory'];

    // 订单首页控制器
    function orderCtrl($scope, $interval, $timeout, $filter, $sce, $anchorScroll, $location, $state, Lightbox, $http, ngAudio, OrderFactory, toaster, ChatImgSelModalFactory, constants,searchOrderModalFactory) {
        $scope.testCompon=function(){
        ...
        $scope.guider_access_token = localStorage['groupHelper_access_token'];
        ...
        $scope.beforeFocusPatientInfo = function() {
            patientInfo = clone($scope.patientInfo);
        };
        ...
        $scope.savePatientInfo = function() {
            if (
                ...
                if ($scope.windowDataListDB.gid) {
                    $http.post(constants.api.order.modifyPatientInfo, {
                        access_token: localStorage['groupHelper_access_token'],
