...
(function() {
    ...
    // 手动注入依赖
    careCtrl.$inject = ['$scope','$state', '$http', 'toaster','DoctorInfoDailogFtory','constants'];

    // 订单首页控制器
    function careCtrl($scope, $state, $http, toaster,DoctorInfoDailogFtory,constants) {
        ...
        function getHandleCareOrder(){
            $http.post(constants.api.care.getHandleCareOrder, {
        ...
        function heathWaitOrderList(){
            $http.post(constants.api.care.heathWaitOrderList, {
                access_token:localStorage['groupHelper_access_token']
            }).
            success(function(data, status, headers, config) {
                if(data.resultCode==1){
        ...
        function receiveCareOrder(item){
            $http.post(constants.api.care.receiveCareOrder, {
                access_token:localStorage['groupHelper_access_token'],
        ...
        $scope.handel=function(item){
            ...
            //获取关怀信息
            $http.post(constants.api.care.getCareOrderDetail, {
                access_token:localStorage['groupHelper_access_token'],
                orderId:item.orderId,
                careType:item.careType,
                fromId:item.fromId
            ...
            //获取医生列表
            $http.post(constants.api.care.getDoctorTeam, {
                access_token:localStorage['groupHelper_access_token'],
        ...
        $scope.closeCare=function(){
            $http.post(constants.api.care.updateCareOrder, {
                access_token:localStorage['groupHelper_access_token'],
                orderId:$scope.currentCare.orderId,
