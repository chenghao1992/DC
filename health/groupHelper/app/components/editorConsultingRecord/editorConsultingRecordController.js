(function() {
    ...
    EditorConsultingRecordModalInstanceCtrl.$inject = ['$scope', '$uibModalInstance', '$state', 'toaster', 'Lightbox', 'EditorConsultingRecordFactory', 'data','ngAudio','$rootScope','DoctorInfoDailogFtory','$http','constants'];

    function EditorConsultingRecordModalInstanceCtrl($scope, $uibModalInstance, $state, toaster, Lightbox, EditorConsultingRecordFactory, data,ngAudio,$rootScope,DoctorInfoDailogFtory,$http,constants) {
        ...
        $scope.diseaseSelectUrl = constants.api.shared.getDisease;
        $scope.diseaseSelectData = [];

        $scope.medicationSelectUrl = constants.api.doctor.getCheckSuggest;
                ...
                if(isPaused){
                    if(item._audio&&item._audio.paused){
                        ...
                        $http({
                            ...
                            method: 'post',
                            headers: {
                                'access-token': localStorage['groupHelper_access_token'],
        ...
        $scope.remainFormat=function(remain){
            if(remain){
                var totalSeconds=remain.toFixed(0);
                var minute=Math.floor(totalSeconds/60)>0?Math.floor(totalSeconds/60)+"'":"";
                var second=totalSeconds%60>0?totalSeconds%60+'"':'';
