(function() {
    ...
    // 手动注入依赖
    threeWayCallingCtrl.$inject = ['$scope', '$state', 'moment', '$timeout', '$stateParams', 'ThreeWayCallingFactory', 'toaster','constants'];

    // 三方通话控制器
    function threeWayCallingCtrl($scope, $state, moment, $timeout, $stateParams, ThreeWayCallingFactory, toaster,constants) {
            ...
            function thenFc(response) {
                if (response) {
                    if(response.status=='37'){
                        if(errorShowTimes==0){
                            toaster.pop('error', null, '余额不足创建三方通话失败');
                        }
                        errorShowTimes++;
                    }
                    if(response.status=='38'){
                        if(errorShowTimes==0){
                            toaster.pop('error', null, '未知原因创建三方通话失败');
                        }
                        errorShowTimes++;
                    }
                    if (response.list){
