(function() {
    angular.module('app')
        .directive('faceIconFilterContent', faceIconFilterContent);

    function faceIconFilterContent($sce) {
        return {
            restrict: 'AE',
            scope: {
                data: '='
            ...
            replace: true,
            template: '<span ng-bind-html="funStrToHTML()" style="word-break: break-all;"></sapn>',
            controller: function ($scope) {
                $scope.funStrToHTML = function () {
                    var faceCode = getFaceCode();
                    var url = 'src/js/filters/faceimages/';
                    var text = $scope.data;
                    if($scope.data !== undefined){
                        for (var i = 0; i < faceCode.length; i++) {
                            text = text.replace(eval('/\\' + faceCode[i].key + '\/g'), '<img style="width:20px;height:20px;margin-bottom: 5px;" src="' + url + faceCode[i].fileName + '" title="' + faceCode[i].fileName + '">');
                            text = text.replace('\n','<br/>');
                        }; 
                    } else {
                        text = '';
