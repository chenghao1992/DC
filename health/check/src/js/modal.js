	...
	return {
		...
		prompt: function(ttl, cnt, fun, opr) {
			...
			if(typeof cnt === 'string'){
				...
				var content = $('<div class="form-text text-md text-center mrt-15 mrb-15"></div>');
				content.append(cnt);
			}
			var operate = $('<div class="dialog-opr-bar clear"></div>');
			var opr_ok = $('<div class="col-md-offset-4 col-md-4"></div>');
			var ok = $('<button type="button" class="w100 btn btn-danger">' + (opr && opr.OK || '确 定') + '</button>');
		...
		confirm: function(ttl, cnt, fun, fun2, opr) {
			var mask = $('<div class="mask"></div>');
			var wrapper = $('<div class="dialog-container animating fade-in-down"></div>');
			var title = $('<div class="dialog-heading font-bold text-center">' + ttl + '</div>');
			var dialog = $('<div class="dialog-body" style="width:auto;min-width:400px"></div>');
			...
			var ok = $('<button type="button" class="w100 btn btn-danger">' + (opr && opr.OK || '确 定') + '</button>');
			var cancel = $('<button type="button" class="w100 btn btn-default">' + (opr && opr.CANCEL || '取 消') + '</button>');
