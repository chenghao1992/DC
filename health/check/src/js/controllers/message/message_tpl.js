 ...
 */
angular.module('app').controller('MsgTplCtrl', MsgTplCtrl);
MsgTplCtrl.$inject = ['$scope', '$timeout','utils','$http','$modal','toaster','$location','$state'];
function MsgTplCtrl($scope, $timeout,utils,$http,$modal,toaster,$location,$state) {
    //所有文档表格
    var articleTable;
    ...
    var editArticle=function(aData){
        $state.go('app.message.tpl_edit',{id:aData.id},{'reload':true});
    };
...
delModalInstanceCtrl.$inject = ['$scope', '$modalInstance'];
function delModalInstanceCtrl($scope, $modalInstance) {
    $scope.ok = function() {
