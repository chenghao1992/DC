'use strict';
(function () {
    app.controller('EditCtrl', ['$scope', '$timeout','utils','$http','$modal','Upload','toaster','$state','$stateParams','$sce',function($scope, $timeout,utils,$http,$modal,Upload,toaster,$state,$stateParams,$sce) {
        $scope.titleLength=0;
            ...
            if(isEdit==true){
                ...
                success(function(data, status, headers, config) {
                    if(data.resultCode==1){
                        ...
                        if($state.includes('app.document.edit_article')){
                            toaster.pop('success','','保存成功,3秒钟后返回文章列表');
                            setTimeout(function(){
                                $state.go('app.document.article_list',{},{reload:true});
                            },3000);
                        }
