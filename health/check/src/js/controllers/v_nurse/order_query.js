...
angular.module('app').controller('OrderQuery', OrderQuery)
OrderQuery.$inject = ['$rootScope', '$scope', '$state', '$timeout', '$http', 'utils', '$stateParams', 'modal'];
function OrderQuery($rootScope, $scope, $state, $timeout, $http, utils, $stateParams, modal) {
        ...
        setTable = function () {
            ...
            dTable = doctorList.dataTable({
                ...
                "columns": [{
                    ...
                    "orderable": false,
                    "render": function(dt, e, data){
                        return dt == '1' ? '待预约' : dt == '2' ? '待支付' : dt == '3' ? '已支付' : dt == '4' ? '已完成' : dt == '5' ? '已取消' : dt == '6' ? '进行中' : dt == '10' ? '预约成功' : '';
                    }
                }]
            });
