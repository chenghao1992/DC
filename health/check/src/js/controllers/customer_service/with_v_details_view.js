'use strict';
(function () {
    app.controller('GroupCheckEditdetails', ['$scope', '$http', '$state', '$rootScope', 'utils', '$stateParams',
            ...
            $scope.submit = function () {
                ...
                $http.post(url, formParam).then(function (resp) {
                    if (resp.data.resultCode === 1) {
                        // 更新界面中的数据
                        if (isPass) {
                            $('#group_check_pass').html(utils.localData('group_check_pass') * 1 + 1);
                        } else {
                            $('#group_check_nopass').html(utils.localData('group_check_nopass') * 1 + 1);
                        }
                        $state.go('app.group_check_list', {type: 'uncheck'}, {});
                    } else {
                        $scope.authError = resp.data.resultMsg;
                    }
                }, function (x) {
