...
angular.module('app').controller('SettlementOfFinance', SettlementOfFinance);
SettlementOfFinance.$inject = ['$rootScope', '$scope', '$state', '$timeout', '$http', 'utils', '$stateParams', 'modal'];
function SettlementOfFinance($rootScope, $scope, $state, $timeout, $http, utils, $stateParams, modal) {
        var name = $stateParams.name,
            date = $stateParams.date,
            datas = null,
            allHaveBankAccount = true,
            userType;
        ...
        $scope.settle = function(dt, flg) {
            ...
            if (typeof dt === 'boolean') {
                ...
                ttl = '<table style="width:100%"><tbody>' +
                    '<tr><td class="w50 p-5 pr-10 text-right">总结算金额</td><td class="pl-10 text-left">' + (dt.noSettleMoney / 100).toFixed(2) + ' 元</td></tr>' +
                    ...
                    (dt.actualMoney * 1 > 0 ?
                    '<tr class="text-primary"><td class="p-5 pr-10 text-right">实际结算金额</td><td class="pl-10 text-left">' + (dt.actualMoney / 100).toFixed(2) + ' 元</td></tr>'
                    :
                    '<tr class="text-primary"><td class="p-5 pr-10 text-right">可结算金额</td><td class="pl-10 text-left">' + (dt.noSettleMoney / 100).toFixed(2) + ' 元</td></tr>')
                    +
                    '</tbody></table>';
            }
                    ...
                    modal.confirm('报表结算', ttl, function() {
                        $http({
                            ...
                            if (resp.data.resultCode == '1' && resp.data.data.result) {
                                if (resp.data.data && resp.data.data.noBankCardIds && resp.data.data.noBankCardIds.length > 0) {
        ...
        $scope.export = function() {
            var url = app.url.finance.downExcel;
            if (name == 'group') {
                var type = 12;
            } else if (name == 'doctor') {
                var type = 11;
                ...
                var type = 13;
                url = app.url.finance.eDownExcel;
            }
            $scope.action_url = url + '?access_token=' + app.url.access_token + '&month=' + date + '&type=' + type + '&pageIndex=' + (index - 1) + '&pageSize=' + total;
            window.location.href = $scope.action_url;
        };
                ...
                dTable = table.dataTable({
                    ...
                    "fnServerData": function(sSource, aoData, fnCallback) {
                        ...
                        $http({
                            ...
                            for (var i = 0; i < resp.pageData.length; i++) {
                                utils.extendHash(resp.pageData[i], ["groupName", "userName", "userRealName", "personNo", "subBankName", "bankName", "bankNo", "noSettleMoney", "actualMoney", "telephone", "status", "storeName", "settlePeriod"]);
                            }
                    ...
                    "createdRow": function(nRow, aData, iDataIndex) {
                        ...
                        ipt.data({
                            'status': aData.status,
                            'id': aData.id,
                            'hasBankAccount': aData.bankNo.length === 0 ? false : true
                        });
                    },
