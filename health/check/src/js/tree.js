(function(window, undefined) {
    var Tree = function(id, settings, fixDt) {
        ...
        this.filter = this.settings.excluded;
        this.allExcluded = false;
        this.p_checked = false;
    ...
    Tree.prototype = {
        ...
        init: function(set) {
            // 配置默认图标
            this.iconArrow = 'fa fa-caret-right/fa fa-caret-down';
            this.iconCheck = 'fa fa-check/fa fa-square';
            this.iconRoot = 'fa fa-hospital-o dcolor';
            this.iconBranch = 'fa fa-h-square dcolor';
            this.iconLeaf = 'fa fa-user-md dcolor';
            this.iconHead = 'headPicFileName';
            this.iconExtra = 'fa fa-h-square dcolor';
            this.iconExcluded = 'fa fa-hospital-o text-muted';

            // 应用自定义图标
            if (this.settings.icons) {
                if (this.settings.icons.arrow) {
                    this.iconArrow = this.settings.icons.arrow;
                ...
                if (this.settings.icons.check) {
                    this.iconCheck = this.settings.icons.check;
                }

                if (this.settings.icons.root) {
                    this.iconRoot = this.settings.icons.root;
                }
                if (this.settings.icons.branch) {
                    this.iconBranch = this.settings.icons.branch;
                }
                if (this.settings.excluded && this.settings.excluded.icon) {
                    this.iconExcluded = this.settings.excluded.icon;
                }
                if (this.settings.icons.leaf) {
                    this.iconLeaf = this.settings.icons.leaf;
                }
                if (this.settings.icons.headPicFileName) {
                    this.iconHead = this.settings.icons.headPicFileName;
                }
            }

            this.iconArrow = this.iconArrow.split('/');
            this.iconArrow_r = this.iconArrow[0];
            this.iconArrow_d = this.iconArrow[1];

            this.iconCheck = this.iconCheck.split('/');
            this.iconCheck_s = this.iconCheck[0];
            this.iconCheck_u = this.iconCheck[1];

            if (this.fixdata && this.fixdata.length > 0) {
                var len = this.fixdata.length;
                for (var i = 0; i < len; i++) {
                    //this.fixdata.push(set.fixdata[i]);
                    this.targetsId.push(this.fixdata[i].id);
                }
                //this.targets = this.fixdata;
            }
            this.targets = this.fixdata;
            if (set.hiddenData && set.hiddenData.length > 0) {
                this.hiddenData = set.hiddenData;
            }
        ...
        // 获取数据，在配置参数中没有传递数据对象的情况下，该方法会根据传递的url获取需要的数据。
        ...
        setTree: function(dt) {
            var that = this;
            //that.settings.targetData = that.targetData;
            that.settings.datakey = that.dataKey;
            ...
            if (this.settings.events && this.settings.events.callback) {
                this.settings.events.callback(this);
            }
        },
        ...
        setBranch: function(data) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (_this.settings.targetData) {
                    data[i] = data[i][_this.settings.targetData] || data[i];
                }
                ...
                // 给‘dt’元素添加一个对象，用于后面读取
                dt.data('info', info);

                if (this.filter) {
                    if (this.filter.type === 'hidden' && this.filter.scope === 'all' && info[this.filter.key.name] == this.filter.key.value) {
                        continue;
                    }
                }

                var _isLeaf = data[i][_this.settings.datakey.leaf];
                subs = data[i][_this.settings.datakey.sub] || (!_isLeaf && typeof _isLeaf !== 'undefined');

                // 根据设定的‘深度’判断次层是否为自定的‘叶子节点’（可能不是真正叶子节点）
                ...
                // 根据设定的‘深度’判断是否到达自定的‘叶子节点’（可能不是真正叶子节点）
                ...
                if ((subs && (ln = subs.length) > 0 && !reached) || (_this.settings.allHaveArr && !_this.isLeaf && !reached)) {
                    ...
                    if ((_this.settings.async && info.id != '0' && !_this.settings.arrType) || (!_this.settings.async && _this.settings.arrType && _this.settings.arrType[_this.treeDepth] != '1')) {
                        iArrow.addClass(this.iconArrow_r);
                    } else {
                        var _search = _this.settings.search && !_this.settings.search.unwind;
                        //if ((_this.settings.arrType && _this.settings.arrType[_this.treeDepth] != '1' && _search) || (_this.settings.arrType[_this.treeDepth] != '1' && _this.settings.async && !_isLeaf)) {
                        if ((_this.settings.arrType && _this.settings.arrType[depth] != '1' && _search) || (_this.settings.arrType && _this.settings.arrType[depth] != '1' && _this.settings.async && !_isLeaf)) {
                            iArrow.addClass(this.iconArrow_r);
                        } else {
                            iArrow.addClass(this.iconArrow_d);
                        }
                    }

                    // 设置图标
                    if (data[i].nodeType !== 'root' && !data[i].isExtra && this.filter) {
                        if (this.filter.type === 'disabled') {
                            if (info[this.filter.key.name] == this.filter.key.value || this.allExcluded) {
                                iIcon.addClass(this.iconExcluded);
                            } else {
                                iIcon.addClass(this.iconBranch);
                            }
                        } else {
                            iIcon.addClass(this.iconBranch);
                        }
                    } else if (data[i].nodeType === 'root') {
                        iIcon.addClass(this.iconRoot);
                    } else {
                        iIcon.addClass(data[i].icon || this.iconExtra);
                    }

                    //iIcon.addClass(data[i].nodeType !== 'root' ? (!data[i].isExtra ? this.iconBranch : (data[i].icon || this.iconExtra)) : this.iconRoot);
                    dt.append(iArrow);
                    ...
                    function arrowHandler(e) {
                        ...
                        _this.p_checked = false;

                        // 异步加载节点数据
                        if (_this.settings.async && !that.hasClass(_this.iconArrow_d) && !e.data.pNode.data('subs') && !_this.settings.loadDisabled) {
                            //setCurLine();
                            //return;
                            ...
                            if (e.data.dt.isExtra) {
                                _dataKey = _this.settings.extra[0].target.dataKey['id'];
                                ...
                                _dataKey = _this.settings.async.target.dataKey['id'];
                                ...
                                success: function(resp) {
                                    ...
                                    if (resp.resultCode === 1 && dt) {
                                        ...
                                        for (var i = 0; i < dt.length; i++) {
                                            if (_this.hiddenData.indexOf(dt[i][_dataKey]) == -1) {
                                                diffDt.push(dt[i]);
                                            }
                                        }
                                        //console.log(_this.hiddenData);
                                        //console.log(diffDt);
                                        //console.log(_dataKey);
                                        _this.isLeaf = true;
                                        e.data.pNode.data('subs', diffDt);
                                        _this.insertNodes(e.data.pNode, diffDt); // 请求到数据后，把新节点插入
                        ...
                        function setCurLine() {
                            ...
                            if (that.hasClass(_this.iconArrow_d)) {
                                pSiblings.addClass('none');
                                that.removeClass(_this.iconArrow_d).addClass(_this.iconArrow_r);
                                if (curDt.length > 0) {
                                ...
                                that.removeClass(_this.iconArrow_r).addClass(_this.iconArrow_d);
                                if (curDt.length > 0) {
                    ...
                    //if (this.settings.async !== true) {
                        if (subs && ln > 0) {
                            _this.treeDepth++;
                            if (this.filter && this.filter.type === 'disabled' && this.filter.scope === 'all' && info[this.filter.key.name] == this.filter.key.value) {
                                this.allExcluded = true;
                                depth = _this.treeDepth;
                            }

                            var objs = this.setBranch(subs); // 创建下一分支

                            if (depth === _this.treeDepth) {
                                this.allExcluded = false;
                            }
                            //this.allExcluded = false;
                    ...
                    var _search = _this.settings.search && !_this.settings.search.unwind;
                    if ((this.settings.async || info.pid != 'x' && info.id != '0' && _this.settings.arrType) && (_this.settings.arrType && _this.settings.arrType[_this.treeDepth] != '1' && _search)) {
                        dd.addClass('none');
                    }

                    if((!_this.settings.async && _this.settings.arrType && _this.settings.arrType[_this.treeDepth] != '1')){
                        dd.addClass('none');
                    }
                    ...
                    hasSub = false;
                    // 设置图标
                    if (this.filter && this.filter.type === 'disabled') {
                        if (info[this.filter.key.name] == this.filter.key.value || this.allExcluded) {
                            iIcon.addClass(this.iconExcluded);
                        } else {
                            iIcon.addClass(this.iconLeaf);
                        }
                    } else {
                        iIcon.addClass(this.iconLeaf);
                    }
                ...
                if (_this.settings.hasCheck) {
                    if (_this.settings.leafCheck && _this.isLeaf && !hasSub) {
                        iCheck.addClass(this.iconCheck_s + ' un-check');
                        dt.append(iCheck);
                        ...
                        $.ajax({
                            ...
                            success: function(resp) {
                                ...
                                if (resp.resultCode === 1 && dt) {
                                    ...
                                    _this.isLeaf = true;
                                    e.pNode.data('subs', diffDt);
                                    _this.insertNodes(e.pNode, diffDt); // 请求到数据后，把新节点插入
                ...
                function checked(e) {
                    ...
                    if(_this.settings.async){
                        _this.p_checked = true;
                    }

                    var _p = e.data.iCheck.parent().parent();
                    if (_this.settings.hasCheck) {
                        if (_this.settings.multiple) {
                            var _chk_class = _this.iconCheck_s.split(' ')[1];
                            var _uchk_class = _this.iconCheck_u.split(' ')[1];
                            ...
                            function setTopCheck(node) {
                                ...
                                if (num === len) {
                                    ...
                                    _check.removeClass('un-check').addClass(_this.iconCheck_u);
                                } else {
                                    //_p.parent().parent().children('dt').trigger('click');
                            ...
                            if (e.data.iCheck.hasClass('un-check')) {
                                if (_this.settings.allCheck) {
                                    ...
                                    // 异步添加所有子节点数据
                                    if (_this.settings.async && !_this.settings.loadDisabled) {
                                        if (e.data.pNode) {
                ...
                if ((!this.filter || info[this.filter.key.name] != this.filter.key.value) && !this.allExcluded) {
                    if (_this.settings.self || (!_this.settings.self && _this.settings.multiple) || (_this.settings.leafCheck && _this.isLeaf && !hasSub)) {
                        dt.on('click', {
                            pNode: dd,
                            iCheck: iCheck,
                            dt: info
                        ...
                        if(_this.p_checked && _this.treeDepth === 0 && _this.settings.async){
                            dt.trigger('click');
                        }
                    }
                }
                /* else {
                                    if(this.filter.type === 'disabled' && this.filter.scope === 'all'){
                                        this.allExcluded = true;
                                    }
                                }*/
        ...
        insertNodes: function(node, dt) {
            ...
            if (this.settings.async && _this.p_checked) {
                node.addClass('none');
            }
